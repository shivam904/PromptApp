    ...
    <p className="font-inter text-sm blue_gradient cursor-pointer" onClick={()=>handleTagClick && handleTagClick(post.tag)}>{post.tag}</p>
   {
    session?.user.id === post.creator._id && pathName==='/profile' && (
      <div className="mt-5 flex justify-between items-center gap-2">
        <button className="outline-none font-satoshi bg-transparent border-none text-black text-sm" onClick={()=>handleEdit(post)}>Edit</button>
        <button className="outline-none font-satoshi bg-transparent border-none text-red-500 text-sm" onClick={()=>handleDelete(post)}>Delete</button>
      </div>
